...
def main(
    ...
    mlflow.set_experiment("SoftTSTCC")

    # Start top‑level run
    run = mlflow.start_run(run_name=f"tstcc_soft_training_{seed}")
    ...
    model_save_path = os.path.join(SAVED_MODELS_PATH, "ECG", str(fs), "TSTCC_soft", pretrain_data, f"{seed}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--soft_instance", action="store_true", default=True)
    parser.add_argument("--soft_temporal", action="store_true", default=True)
    parser.add_argument("--tau_temp", type=float, default=1)
    parser.add_argument("--tau_inst", type=float, default=10)
    parser.add_argument("--dist_type", type=str, default="euc")
