...
from models.tstcc_soft import data_generator_from_arrays, Trainer_wo_DTW, tstcc_soft, TC, Config as ECGConfig, \
    train_linear_classifier, evaluate_classifier, encode_representations, show_shape, \
        build_tstcc_fingerprint, search_encoder_fp, build_linear_loaders
...
class ECGTSTCCFlow(FlowSpec):
    ...
    # soft version
    lambda_aux = Parameter("lambda_aux", default=0.2)
    ...
    tau_inst = Parameter("tau_inst", default=10)
    ...
    def train_tstcc(self):
        ...
        if run_id:
            ...
            uri = f"runs:/{run_id}/tstcc_soft_model"
            ...
            self.model = tstcc_soft(self.configs).to(self.device)
            ...
            with mlflow.start_run(run_id=self.mlflow_run_id):
                ...
                # store checkpoint
                ckpt = os.path.join(run_dir, "tstcc_soft.pt")
                torch.save(
                    {
