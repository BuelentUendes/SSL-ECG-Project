...
def main(
    ...
    mlflow.set_experiment(f"SoftTSTCC with CV {classifier_model}")

    # Start top‑level run
    run = mlflow.start_run(run_name=f"tstcc_soft_cv_{classifier_model}_{seed}_lf_{label_fraction}")
    ...
    model_save_path = os.path.join(
        SAVED_MODELS_PATH, "ECG", str(fs), "TSTCC_soft", pretrain_data, f"{seed}", f"{window_size}", f"{step_size}",
        str(train_ratio_encoder)
    )
    results_save_path = os.path.join(
        RESULTS_PATH, "ECG", "TSTCC_soft", classifier_model, f"{seed}", f"{label_fraction}", f"{window_size}",
    ...
    # ── Step 2: TS‑TCC Soft Pretraining ─────────────────────────────────────────
    torch.cuda.empty_cache()
    set_seed(seed)

    # Fingerprint & search
    fp = build_tstcc_fingerprint({
        "model_name": "TSTCC_soft",
        "seed": seed,
    ...
    if (cached or os.path.exists(os.path.join(model_save_path, "tstcc_soft.pt"))) and not (force_retraining):
        ...
        cfg.soft_instance = soft_instance
        cfg.soft_temporal = soft_temporal
        cfg.tau_temp = tau_temp
        cfg.tau_inst = tau_inst
        cfg.dist_type = dist_type
        cfg.lambda_ = lambda_

        model = tstcc_soft(cfg).to(device)
        ...
        print("No cached encoder; training TS-TCC Soft from scratch")
        cfg = ECGConfig(fs, window_size)
        cfg.num_epoch = tcc_epochs
        cfg.batch_size = tcc_batch_size
        cfg.TC.timesteps = tc_timesteps
        ...
        model = tstcc_soft(cfg).to(device)
        tc_head = TC(cfg, device).to(device)
        opt_m = optim.Adam(model.parameters(), lr=tcc_lr, weight_decay=3e-4)
        opt_tc = optim.Adam(tc_head.parameters(), lr=tcc_lr, weight_decay=3e-4)

        mlflow.log_params(fp)
        workdir = tempfile.mkdtemp(prefix="tstcc_soft_")
        Trainer_wo_DTW(
            model=model,
        ...
        ckpt = os.path.join(workdir, "tstcc_soft.pt")
        torch.save(
        ...
        mlflow.log_artifact(ckpt, artifact_path="tstcc_soft_model")

        saved_results = os.path.join(model_save_path, "tstcc_soft.pt")
        torch.save(
            {"encoder": model.state_dict(),
             "tc_head": tc_head.state_dict()},
            saved_results
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="TS-TCC Soft Training Pipeline with CV and Logistic Regression",
    ...
    data_group.add_argument("--train_ratio_encoder", default=0.75, type=float,
                            help="If set to 0.75, it will result in 60/20/20 split and have a validation set for TSTCC Soft,"
                                 "Alternatively, set to 1.0 to train on all unlabelled training instances.")

    # ══════════════════════════════════════════════════════════════════════════════
    # TS-TCC Soft Encoder Training
    # ══════════════════════════════════════════════════════════════════════════════
    tstcc_group = parser.add_argument_group('TS-TCC Soft Encoder Training')
    tstcc_group.add_argument("--tcc_epochs", type=int, default=40,
                            help="Number of epochs for TS-TCC Soft pretraining")
    tstcc_group.add_argument("--tcc_lr", type=float, default=3e-4,
                            help="Learning rate for TS-TCC Soft training")
    tstcc_group.add_argument("--tcc_batch_size", type=int, default=128,
                            help="Batch size for TS-TCC Soft training")

    # TS-TCC Architecture Parameters
    tstcc_arch_group = parser.add_argument_group('TS-TCC Soft Architecture')
    ...
    tstcc_soft_group = parser.add_argument_group('Soft Contrastive Learning Parameters')
    tstcc_soft_group.add_argument("--lambda_aux", type=float, default=0.2,
                                 help="Lambda auxiliary parameter")
    tstcc_soft_group.add_argument("--soft_instance", action="store_true", default=True,
                                 help="Enable soft instance contrastive learning")
    tstcc_soft_group.add_argument("--soft_temporal", action="store_true", default=True,
                                 help="Enable soft temporal contrastive learning")
    tstcc_soft_group.add_argument("--tau_temp", type=float, default=1,
                                 help="Temperature parameter for temporal contrastive learning")
    tstcc_soft_group.add_argument("--tau_inst", type=float, default=10,
                                 help="Temperature parameter for instance contrastive learning")
    tstcc_soft_group.add_argument("--dist_type", type=str, default="euc",
                                 help="Distance type for soft labels")
    tstcc_soft_group.add_argument("--lambda_", type=float, default=0.5,
                                 help="Lambda parameter for balancing losses")
