...
def load_ssl_runtime_data(results_base_path="results/ECG"):
    ...
    # IMPORTANT: Here we only use 5 seeds as we need to make sure that all methods are run on the same machine
    ...
    for method, seeds in methods_seeds.items():
        ...
        for seed in seeds:
            ...
            base_path = os.path.join(RESULTS_PATH, "ECG", method, "logistic_regression", str(seed), "0.1", "10", "5",
                                     "1.0")
...
def create_ssl_runtime_plot(legend_style='in_labels', save_path=None, figsize=(10, 8),
    ...
    for method in ssl_data.keys():
        ...
        # Try to load from multiple seeds -> for model performance we use all 10 seeds, for runtime and memory only 5 seeds
        seeds = [3, 5, 7, 9, 42, 11, 13, 15, 17, 19]
        for seed in seeds:
    ...
    with plt.style.context(['ieee']):
        ...
        method_colors = {
            'TS-TCC': '#0E9FEB',  # Light blue
            'TS-TCC+S3': '#005377',  # Darker blue
        ...
        for _, row in method_averages.iterrows():
            method = row['method']
            method = method.replace("TSTCC", "TS-TCC")  # Convert display name back to directory name
            x = row['avg_runtime']
            y = row['auroc']
        ...
        for _, row in method_averages.iterrows():
            ...
            if legend_style == 'in_labels':
                label_text = f"{method} [{round(memory, 2)} GB, {round(x)}s]"
            else:
                label_text = method
        ...
        # Save the plots
        save_name_pdf = 'ssl_runtime_analysis_v2.pdf'
        save_name_png = 'ssl_runtime_analysis_v2.png'
        if save_path:
        ...
        for _, row in method_averages.iterrows():
            print(f"{row['method']}: Runtime={row['avg_runtime']:.2f}s, "
                  f"Memory={row['avg_memory']:.2f} GB, AUROC={row['auroc']:.3f} (from {row['num_seeds']} seeds for memory)")
