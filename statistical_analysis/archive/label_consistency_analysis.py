    ...
    def evaluate_with_participant_cv(
        ...
        for metric, values in fold_results.items():
            if len(values) > 0:
                ...
                results[f'{metric}_se'] =  values.std()/np.sqrt(len(values))
                results[f'{metric}_std'] = values.std()
            else:
                results[f'{metric}_mean'] = np.nan
                results[f'{metric}_se'] = np.nan
...
def main():
    ...
    for pair_name, pair_results in results.items():
        ...
        if 'overall_discrimination' in pair_results:
            ...
            print(f"      Balanced Accuracy: {overall['balanced_accuracy_mean']:.3f} ± {overall['balanced_accuracy_se']:.3f}")
            print(f"      ROC-AUC (standard error): {overall['roc_auc_mean']:.3f} ± {overall['roc_auc_se']:.3f}")
            print(f"      PR-AUC (standard error): {overall['pr_auc_mean']:.3f} ± {overall['pr_auc_se']:.3f}")
            print(f"      Accuracy (standard error): {overall['accuracy_mean']:.3f} ± {overall['accuracy_se']:.3f}")
            print(f"    DUMMY CLASSIFIER (Majority Class):")
            print(f"      Balanced Accuracy: {overall['dummy_balanced_accuracy_mean']:.3f} ± {overall['dummy_balanced_accuracy_se']:.3f}")
            print(f"      ROC-AUC: {overall['dummy_roc_auc_mean']:.3f} ± {overall['dummy_roc_auc_se']:.3f}")
            print(f"      PR-AUC: {overall['dummy_pr_auc_mean']:.3f} ± {overall['dummy_pr_auc_se']:.3f}")
            print(f"      Accuracy: {overall['dummy_accuracy_mean']:.3f} ± {overall['dummy_accuracy_se']:.3f}")
        ...
        if 'per_class_discrimination' in pair_results:
            ...
            if 'class_0_baseline' in per_class:
                ...
                print(f"  \n  CLASS 0 (BASELINE) DISCRIMINATION:")
                print(f"    LOGISTIC REGRESSION:")
                print(f"      Balanced Accuracy: {class0['balanced_accuracy_mean']:.3f} ± {class0['balanced_accuracy_se']:.3f}")
                print(f"      ROC-AUC: {class0['roc_auc_mean']:.3f} ± {class0['roc_auc_se']:.3f}")
                print(f"      PR-AUC: {class0['pr_auc_mean']:.3f} ± {class0['pr_auc_se']:.3f}")
                print(f"    DUMMY CLASSIFIER:")
                print(f"      Balanced Accuracy: {class0['dummy_balanced_accuracy_mean']:.3f} ± {class0['dummy_balanced_accuracy_se']:.3f}")
                print(f"      ROC-AUC: {class0['dummy_roc_auc_mean']:.3f} ± {class0['dummy_roc_auc_se']:.3f}")
                print(f"      PR-AUC: {class0['dummy_pr_auc_mean']:.3f} ± {class0['dummy_pr_auc_se']:.3f}")
            ...
            if 'class_1_stress' in per_class:
                ...
                print(f"    LOGISTIC REGRESSION:")
                print(f"      Balanced Accuracy: {class1['balanced_accuracy_mean']:.3f} ± {class1['balanced_accuracy_se']:.3f}")
                print(f"      ROC-AUC: {class1['roc_auc_mean']:.3f} ± {class1['roc_auc_se']:.3f}")
                print(f"      PR-AUC: {class1['pr_auc_mean']:.3f} ± {class1['pr_auc_se']:.3f}")
                print(f"    DUMMY CLASSIFIER:")
                print(f"      Balanced Accuracy: {class1['dummy_balanced_accuracy_mean']:.3f} ± {class1['dummy_balanced_accuracy_se']:.3f}")
                print(f"      ROC-AUC: {class1['dummy_roc_auc_mean']:.3f} ± {class1['dummy_roc_auc_se']:.3f}")
                print(f"      PR-AUC: {class1['dummy_pr_auc_mean']:.3f} ± {class1['dummy_pr_auc_se']:.3f}")
