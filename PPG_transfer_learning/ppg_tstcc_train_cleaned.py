# Script to showcast the transferability of the approach
# We have a pretrained ECG representation and apply it downstream to PPG
# Check if this helps the whole procedure or not
...
def main(
    ...
    transfer_ecg_representation: bool,
):
    # ── Step 0: Setup ────────────────────────────────────────────────────────────
    set_seed(seed)
    ...
    if (cached or any(os.path.exists(f) for f in model_files)) and not force_retraining:
        ...
        elif transfer_ecg_representation:
            print("We found a pretrained ECG model and ECG transfer is set to True. Load the pretrained weights")
            ckpt_path = os.path.join(ecg_model_save_path, "tstcc.pt")
        else:
