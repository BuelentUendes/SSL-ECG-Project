# Code taken from:
# https://github.com/klean2050/tiles_ecg_model/blob/master/src/models/s4_model.py
...
class S4DKernel(nn.Module):
    ...
    def forward(self, L):
        ...
        ts = torch.arange(L, device=A.device, dtype=dt.dtype)  # float, not int64
        K = dtA.unsqueeze(-1) * ts  # (H N L)
...
class S4Model(nn.Module):
    def __init__(
        ...
        # Use S4D directly (not S4Block) so state dict keys match the pretrained checkpoint.
        self.s4_layers = nn.ModuleList()
        self.norms = nn.ModuleList()
        self.dropouts = nn.ModuleList()
        for _ in range(n_layers):
            self.s4_layers.append(
                S4D(d_model, dropout=dropout, transposed=True)
            )
...
if __name__ == "__main__":
    # testing the model
    model = S4Model(1, d_output=128, d_model=256, n_layers=4, prenorm=True)
