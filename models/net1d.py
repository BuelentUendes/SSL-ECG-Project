...
Original author: Shenda Hong, Mar 2020
...
class BasicStage(nn.Module):
    ...
    def forward(self, x):
        ...
        for i_block in range(self.m_blocks):
            ...
            out = net(out)
            if self.verbose:
                print('stage: {}, block: {}, in_channels: {}, out_channels: {}, outshape: {}'.format(
                    self.i_stage, i_block, net.in_channels, net.out_channels, list(out.shape)))
...
class Net1D(nn.Module):
    ...
    Output:
        ...
        deep_features: (n_samples, filter_list[-1])  — only when return_features=True

    params:
        in_channels, base_filters, ratio,
        filter_list, m_blocks_list,
        kernel_size, stride, groups_width,
        n_classes, use_bn, use_do,
        return_features, verbose
    """
