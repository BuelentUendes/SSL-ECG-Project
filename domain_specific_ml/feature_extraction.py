...
def get_non_linear_features(rpeaks: np.ndarray, sampling_rate: int, window_size: int = 30):
    ...
    hrv_nonlinear_features = _hrv_nonlinear_fragmentation(rri, rri_time, rri_missing=rri_missing,
                                                          out=hrv_nonlinear_features)
...
def extract_features_from_window(ecg_window: np.ndarray, sampling_rate: int, window_size: int) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        # If window size is small, then restricting to 12 R peaks in time window is too restrictive
        if len(r_peaks) < 12 and window_size >=30:
        ...
        non_linear_features = get_non_linear_features(r_peaks, sampling_rate, window_size)
        ...
        for key, value in frequency_features.items():
            if window_size <30:
                if ("uhf" in key) and ("relative_band" not in key):
...
def process_data(h5_file_path: str, output_path: str, sampling_rate: int = 1000, window_size: int=30):
    """
    Process windowed data and extract features, maintaining windowed structure.

    Args:
        h5_file_path: Path to the windowed H5 file
            ...
            for condition in participant_group.keys():  # baseline, mental_stress, etc.
                ...
                for segment_name in condition_group.keys():  # segment_0, segment_1, etc.
                    ...
                    for window_idx in range(n_windows):
                        ...
                        window_features = extract_features_from_window(ecg_window, sampling_rate, window_size)

                        if window_features is not None:
                            # Remove metadata keys, keep only feature values
                            feature_dict = {k: v for k, v in window_features.items()
                                            if
                                            k not in ['participant_id', 'condition', 'segment', 'window_idx', 'label']}
                    ...
                    if segment_features_list:
                        ...
                        print(
                            f"  {segment_name}: {segment_features_array.shape} ({valid_windows}/{n_windows} valid windows)")
                    else:
...
def main(args):
    """
    Main function to extract features from dataset.
    """

    if args.dataset == "ours":
        input_path_folder = "interim/ECG"
        output_path_folder = "interim/ECG_features" if args.use_normalized_ecg_signal else "interim/ECG_features_unnormalized"
        ...
        input_path_folder = "interim/WESAD/ECG"
        output_path_folder = "interim/WESAD_features/ECG" if args.use_normalized_ecg_signal else "interim/WESAD_features_unnormalized/ECG"
    ...
    print(f"Processing data from: {h5_file_path}")
    print(f"Output directory: {output_path}")

    process_data(h5_file_path, output_path, args.sample_frequency, args.window_size)
