...
def main(
    ...
    model_save_path = os.path.join(
        SAVED_MODELS_PATH, "WESAD", f"{modality}", str(fs), f"{model_type}", f"{seed}", f"{label_fraction}"
    )
    create_directory(model_save_path)

    # Data path
    window_data_path = os.path.join(DATA_PATH, "interim", "WESAD", f"{modality}", 'windowed_data.h5')
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Train ECG classifier for the WESAD Dataset.")
